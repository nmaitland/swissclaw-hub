...
describe('Kanban API', () => {
  beforeEach(async () => {
    ...
    // Don't clean users as they're needed for foreign key constraints
  });
  ...
  describe('PUT /api/kanban/:id', () => {
    it('should update an existing kanban task', async () => {
      ...
      const newTask = await db.KanbanTask.create({
        ...
        description: 'Description for test task update',
        priority: 'medium',
      ...
      const response = await request(app)
        .put(`/api/kanban/${newTask.id}`)
        .send(updateData)
        .expect(200);

      expect(response.body.id).toBe(newTask.id);
  ...
  describe('DELETE /api/kanban/:id', () => {
    it('should delete an existing kanban task', async () => {
      ...
      const newTask = await db.KanbanTask.create({
        ...
        description: 'Description for test task delete',
        priority: 'medium',
        assigned_to: 'testuser',
        column: 'todo'
      });
