...
let cachedProviderPath;
function getProviderPath() {
    if (cachedProviderPath !== undefined)
        return cachedProviderPath;
    try {
        // Ask OpenClaw directly — slow but authoritative. Result is cached after first call.
        const output = execFileSync("openclaw", ["config", "get", "agents.defaults.workspace"], { encoding: "utf-8", timeout: 10_000 });
        ...
        if (workspacePath && path.isAbsolute(workspacePath)) {
            cachedProviderPath = path.join(workspacePath, ".openclaw", "credentials", "swissclaw-hub.ts");
            return cachedProviderPath;
        }
    }
    catch {
    ...
    cachedProviderPath = path.join(OPENCLAW_DIR, ".openclaw", "credentials", "swissclaw-hub.ts");
    return cachedProviderPath;
}
export async function loadCredentials() {
    ...
    if (!username || !password) {
        throw new Error("Hub credentials not configured. Either:\n" +
